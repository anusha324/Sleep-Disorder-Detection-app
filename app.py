...
<style>
    ...
    /* Global styles */
    .stApp {
        background: linear-gradient(135deg, #f8f9ff 0%, #e8f2ff 25%, #f0f8ff 50%, #f5f9ff 75%, #fafbff 100%);
    ...
    .main-header {
        font-size: 3.5rem;
        background: linear-gradient(45deg, #2c3e50, #34495e, #2980b9, #3498db, #5dade2);
    ...
    /* Specific targeting for form labels and content */
    ...
    /* Prediction result specific styling */
    ...
    /* Force all text in main content area to be dark */
    .main .block-container * {
        color: #2c3e50 !important;
    }
    ...
    /* Force all text in the main area to be dark */
    .main * {
        color: #2c3e50 !important;
    }
    
    /* Exception for success/warning/error/info boxes */
    .stSuccess *,
    .stWarning *,
    .stError *,
    .stInfo * {
...
def show_home(model, scaler, bmi_encoder, gender_encoder, reverse_disorder_mapping):
    ...
    with col1:
        st.markdown('<h2 class="sub-header" style="color: #2c3e50 !important;">🔍 Enter Your Information</h2>', unsafe_allow_html=True)
...
def show_prediction_result():
    ...
    st.markdown(f'<h3 style="color: #2c3e50 !important; font-size: 2rem; margin: 1rem 0;">{predicted_disorder}</h3>', unsafe_allow_html=True)
    st.markdown('<h3 class="sub-header" style="color: #2c3e50 !important;">💡 Recommendations</h3>', unsafe_allow_html=True)
